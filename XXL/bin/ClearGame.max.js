...
var ClearGame=(function(){
	...
	__class(ClearGame,'xxl.ClearGame');
	var __proto=ClearGame.prototype;
	...
	__proto.checkData=function(){
		for (var i=0;i < this.mapData.length;i++){
			var hasBlick=false;
			hasBlick=this.checkLine(this.mapData[i]);
			if (hasBlick){
				return true;
			}
			hasBlick=this.checkLine(this.getOneRow(i));
			if (hasBlick){
				return true;
			}
		}
	...
	//遍历查找
	__proto.checkLine=function(arr){
		var last=0;
	...
	//得到一行
	__proto.getOneRow=function(row){
		var rowArr=[];
		for (var i=0;i < this.mapData.length;i++){
			rowArr.push(this.mapData[i][row]);
		}
		return rowArr;
	}

	//返回没有可消除的数据
	__proto.initBlick=function(){
		var ret=new Array(this.SIZE);
		for (var i=0;i < ret.length;i++){
			ret[i]=new Array(this.SIZE);
	...
	__proto.initView=function(mapData){
		console.log(mapData);
		this.container.pos(50 ,300);
		this.container.graphics.drawRect(0 ,-310 ,500 ,810 ,null ,"#ffffff");
		Laya.stage.addChild(this.container);
		for (var i=0;i < mapData.length;i++){
			var arr=mapData[i];
			this.map[i]=new Array(arr.length);
			for (var j=0;j < arr.length;j++){
				var color=mapData[i][j];
				var b=new Blick(color ,i ,j);
				b.pos(i *100 ,j *100);
				this.map[i][j]=b;
				this.container.addChild(b);
				b.on("click" ,this ,this.onClickBlick);
			}
		}
	}

	__proto.onClickBlick=function(e){
		var item=e.target;
		if (this.a==null){
			this.a=item;
			this.a.isSelect=true;
			}else if (this.a !=null && this.a !=item && this.b==null){
			this.b=item;
			this.b.isSelect=true;
		}
		if (this.a !=null && this.b !=null){
			Laya.timer.once(500 ,this ,this.changePos);
		}
	}
	...
	__proto.changePos=function(){
		...
		var bi=this.b.i;
		var bj=this.b.j;
		this.map[ai][aj]=this.b;
		this.map[bi][bj]=this.a;
		this.a.i=bi;
		this.a.j=bj;
		this.b.i=ai;
		this.b.j=aj;
		...
		Laya.timer.once(1000 ,this ,this.checkClear);
	}

	//遍历查找
	__proto.findClear=function(arr){
		var findResult=[];
		var result=[];
		var lastBlick=arr[0];
		var curBlick;
		for (var i=1;i < arr.length;i++){
			...
			if (lastBlick.color==curBlick.color){
				...
				this.saveResult(result ,findResult);
				lastBlick=curBlick;
				result=[];
			}
		}
	...
	//去重
	__proto.saveResult=function(arr,findResult){
		if (arr.length >=3){
			var blick;
			for(var $each_blick in arr){
				blick=arr[$each_blick];
				if (findResult.indexOf(blick)< 0){
	...
	__proto.filterCB=function(element,index,self){
	...
	__proto.checkClear=function(){
		...
		for (var i=0;i < this.map.length;i++){
			clearList=clearList.concat(this.findClear(this.map[i]));
			clearList=clearList.concat(this.findClear(this.getOneClickRow(i)));
		}
		clearList=clearList.filter(this.filterCB);
		console.log("checkClear()");
		console.log(clearList);
		if (clearList.length >=3){
			while (clearList.length > 0){
				var item=clearList.pop();
				this.map[item.i][item.j]=null;
				item.removeSelf();
			}
			this.resetMap();
			}else {
	...
	__proto.resetMap=function(){
		var latestTime=0;
		for (var i=0;i < this.map.length;i++){
			var mapCol=this.map[i];
			this.map[i]=this.resetCol(mapCol ,i);
			for (var j=mapCol.length-1;j >=0;j--){
				var item=this.map[i][j];
				if (item && item.j !=j){
					item.j=j;
					var time=(j *100-item.y)/ this.SPEED;
					if (time > latestTime){
						latestTime=time;
					}
					this.Animation(item ,j *100 ,time);
				}else {}
			}
		}
		Laya.timer.once(latestTime+200 ,this ,this.checkClear);
	}

	__proto.resetCol=function(arr,col){
		var ret=[];
		...
		for (var k=count-1;k >=0;k--){
			var item=ret[k];
			if (item==null){
				var color=Math.floor(Math.random()*Blick.allColor.length);
				item=new Blick(color ,col ,posY);
				this.container.addChild(item);
				item.on("click" ,this ,this.onClickBlick);
				ret[k]=item;
	...
	//距 离除 时间
	__proto.Animation=function(target,targetY,time){
		(time===void 0)&& (time=1000);
		Tween.to(target ,{y:targetY},time);
	}

	__proto.getOneClickRow=function(row){
		var rowArr=[];
		for (var i=0;i < this.map.length;i++){
			rowArr.push(this.map[i][row]);
		}
		return rowArr;
	}
...
//class xxl.Blick extends laya.display.Sprite
var Blick=(function(_super){
	function Blick(color,col,row,targetY,dur){
		//大小
		this.i=0;
		this.j=0;
		this.color=0;
		...
		this.selSp=null;
		Blick.__super.call(this);
		(targetY===void 0)&& (targetY=0);
		(dur===void 0)&& (dur=0);
		this.mouseEnabled=true;
		this.size(100,100);
		this.i=col;
		this.j=row;
		this.color=color;
		if (color >=0 && color < Blick.allColor.length){
			var c=Blick.allColor[color];
			this.graphics.clear();
			this.graphics.drawRect(1,1,100-2,100-2,c);
		}
		this.selSp=new Sprite();
		this.selSp.graphics.drawCircle(100 / 2,100 / 2,30,"#000000");
		this.selSp.alpha=0.5;
	}

	__class(Blick,'xxl.Blick',_super);
	var __proto=Blick.prototype;
	__getset(0,__proto,'isSelect',function(){
		return this._isSelect;
		},function(b){
		this._isSelect=b;
		if (b){
			this.addChild(this.selSp);
			}else {
			this.selSp.removeSelf();
		}
	});

	Blick.SIZE=100;
	__static(Blick,
	['allColor',function(){return this.allColor=["#ff0000","#00ff00","#0000ff"];}
	]);
	return Blick;
})(Sprite)
